...
class CalendarIndexer(TimeIndexer):
    ...
    def __init__(self: Self, hass: HomeAssistant, entity_id: str) -> None:
        ...
        self.name: str = f"calendar:{entity_id}"
    ...
    async def key(self: Self, ts: datetime) -> int:
        ...
        Args:
            ts: The timestamp to check for calendar events.
        ...
        # Using a 1-second window to check if any event is active at ts
        start_time = ts
    ...
    async def next_boundary(self: Self, ts: datetime) -> datetime:
        ...
        Args:
            ts: The timestamp to find the next boundary after.
        ...
        try:
            ...
            # Collect all event boundaries (start and end times) after ts
            boundaries = []
            for event in events:
                # Parse event times
