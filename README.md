# homeassistant-binary-state-forecaster
...
## Fading parameter
The fading parameter should be between `0..1`. This controls how much the past is important. Higher values means sensor will use past observations with much higher rate in the prediction, which means the forecaster sensor will learn the new patterns changes slower. Lower values means the sensor forgets the past easier and use the observations from the near future with much higher weight. This will make the forecaster more sensitive to changes and makes it faster to learn new patterns but could also spoils the predictions if there is an outlier day.

## Threshold parameter
The integration contains a threshold parameter which defines the minimum calculated probability when the forecaster sensor state becomes `on`. If the calculated probability for the current time block is less then the threshold the forecaster sensor state will be `off`.
...
So at time $T$ you want to know the predicted value for the $T + 15 minute$ you can get from the `state_attr("binary_sensor.forecaster", "probabilities")[15 // 5]`.
