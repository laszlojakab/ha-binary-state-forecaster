...
class TestInitialization:
    ...
    async def test_init_with_simple_forecaster(self) -> None:
...
class TestFirstUpdate:
    ...
    async def test_first_update_records_state(self) -> None:
    ...
    async def test_first_update_no_forecaster_training(self) -> None:
        """Test that first update doesn't train forecaster."""
        indexer = CompositeIndexer([TimeOfDayIndexer(60)])
        forecaster = TimeAwareForecaster(indexer)
        tracker = StateTracker(forecaster)

        await tracker.update(datetime(2024, 1, 1, 10, 0), "on")
    ...
    async def test_first_update_various_state_types(self) -> None:
        ...
        await tracker1.update(datetime(2024, 1, 1, 10, 0), "active")
        assert tracker1.last_state == "active"

        # Integer state
        tracker2 = StateTracker(forecaster)
        await tracker2.update(datetime(2024, 1, 1, 10, 0), 1)
        assert tracker2.last_state == 1

        # Boolean state
        tracker3 = StateTracker(forecaster)
        await tracker3.update(datetime(2024, 1, 1, 10, 0), True)
...
class TestStateTransitions:
    ...
    async def test_second_update_creates_interval(self) -> None:
        ...
        await tracker.update(start, "on")
        await tracker.update(end, "off")
    ...
    async def test_multiple_transitions(self) -> None:
        ...
        await tracker.update(datetime(2024, 1, 1, 9, 0), "on")
        await tracker.update(datetime(2024, 1, 1, 10, 0), "off")
        await tracker.update(datetime(2024, 1, 1, 11, 0), "on")
        await tracker.update(datetime(2024, 1, 1, 13, 0), "idle")
        ...
        pred1 = await forecaster.predict(datetime(2024, 1, 1, 9, 30))
        assert pred1.state == "on"

        # At 10:30 should predict "off"
        pred2 = await forecaster.predict(datetime(2024, 1, 1, 10, 30))
        assert pred2.state == "off"

    @pytest.mark.asyncio


    async def test_alternating_states(self) -> None:
        ...
        for i in range(10):
            ...
            await tracker.update(base + timedelta(minutes=i * 5), state)
...
class TestIntervalCalculation:
    ...
    async def test_short_interval(self) -> None:
    ...
    async def test_long_interval(self) -> None:
        ...
        prediction = await forecaster.predict(datetime(2024, 1, 1, 14, 0))
        assert prediction.state == "on"

    @pytest.mark.asyncio


    async def test_interval_spanning_midnight(self) -> None:
        ...
        await tracker.update(start, "sleeping")
        await tracker.update(end, "awake")
    ...
    async def test_interval_spanning_month(self) -> None:
        ...
        await tracker.update(start, "active")
        await tracker.update(end, "idle")
...
class TestTimestampEdgeCases:
    ...
    async def test_equal_timestamps(self) -> None:
        ...
        await tracker.update(ts, "on")
        await tracker.update(ts, "off")  # Same timestamp
    ...
    async def test_backward_timestamp(self) -> None:
        ...
        await tracker.update(datetime(2024, 1, 1, 11, 0), "on")
        await tracker.update(datetime(2024, 1, 1, 10, 0), "off")  # Earlier time
    ...
    async def test_leap_year_date(self) -> None:
...
class TestStateTypes:
    ...
    async def test_string_states(self) -> None:
        ...
        await tracker.update(datetime(2024, 1, 1, 11, 0), "active")
        await tracker.update(datetime(2024, 1, 1, 12, 0), "sleeping")
    ...
    async def test_integer_states(self) -> None:
        """Test tracking with integer states."""
        indexer = CompositeIndexer([TimeOfDayIndexer(60)])
        forecaster = TimeAwareForecaster(indexer)
        tracker = StateTracker(forecaster)

        await tracker.update(datetime(2024, 1, 1, 10, 0), 0)
        await tracker.update(datetime(2024, 1, 1, 11, 0), 1)
        await tracker.update(datetime(2024, 1, 1, 12, 0), 2)
    ...
    async def test_boolean_states(self) -> None:
        """Test tracking with boolean states."""
        indexer = CompositeIndexer([TimeOfDayIndexer(60)])
        forecaster = TimeAwareForecaster(indexer)
        tracker = StateTracker(forecaster)

        await tracker.update(datetime(2024, 1, 1, 10, 0), True)
        await tracker.update(datetime(2024, 1, 1, 11, 0), False)
        await tracker.update(datetime(2024, 1, 1, 12, 0), True)
    ...
    async def test_tuple_states(self) -> None:
        """Test tracking with tuple states (composite states)."""
        indexer = CompositeIndexer([TimeOfDayIndexer(60)])
        forecaster = TimeAwareForecaster(indexer)
        tracker = StateTracker(forecaster)

        await tracker.update(datetime(2024, 1, 1, 10, 0), ("mode", "heating"))
        await tracker.update(datetime(2024, 1, 1, 11, 0), ("mode", "cooling"))
        await tracker.update(datetime(2024, 1, 1, 12, 0), ("mode", "off"))
    ...
    async def test_mixed_state_types(self) -> None:
...
class TestSameStateAccumulation:
    ...
    async def test_same_state_repeated(self) -> None:
        ...
        await tracker.update(datetime(2024, 1, 1, 10, 0), "on")
        await tracker.update(datetime(2024, 1, 1, 11, 0), "on")
        await tracker.update(datetime(2024, 1, 1, 12, 0), "on")
        await tracker.update(datetime(2024, 1, 1, 13, 0), "off")

        assert tracker.last_state == "off"

        # Forecaster should have accumulated "on" time
        prediction = await forecaster.predict(datetime(2024, 1, 1, 11, 30))
        assert prediction.state == "on"

    @pytest.mark.asyncio


    async def test_same_state_many_updates(self) -> None:
        ...
        base = datetime(2024, 1, 1, 10, 0)
        for i in range(20):
            await tracker.update(base + timedelta(minutes=i), "constant")
...
class TestForecasterIntegration:
    ...
    async def test_with_time_of_day_indexer(self) -> None:
        ...
        for day in range(7):
            ...
            await tracker.update(base_date, "on")
            await tracker.update(base_date + timedelta(hours=1), "off")

        # Predict at 9:30 should be "on"
        prediction = await forecaster.predict(datetime(2024, 1, 8, 9, 30))
        assert prediction.state == "on"

    @pytest.mark.asyncio


    async def test_with_day_of_week_indexer(self) -> None:
        ...
        for day in range(5):  # Monday to Friday
            ...
            await tracker.update(base, "work")
            await tracker.update(base + timedelta(hours=8), "home")

        # Sat-Sun: relax mode
        for day in range(2):  # Saturday and Sunday
            base = datetime(2024, 1, 6 + day, 9, 0)
            await tracker.update(base, "relax")
            await tracker.update(base + timedelta(hours=8), "home")

        # Predict Monday at 9:30 should be "work"
        monday_pred = await forecaster.predict(datetime(2024, 1, 8, 9, 30))
        assert monday_pred.state == "work"

    @pytest.mark.asyncio


    async def test_with_hierarchical_indexer(self) -> None:
        ...
        await tracker.update(datetime(2024, 1, 1, 10, 0), "winter_weekday_morning")
        await tracker.update(datetime(2024, 1, 1, 18, 0), "winter_weekday_evening")
        await tracker.update(datetime(2024, 6, 15, 10, 0), "summer_weekday_morning")
    ...
    async def test_with_custom_half_life(self) -> None:
        ...
        await tracker.update(datetime(2024, 1, 1, 10, 0), "on")
        await tracker.update(datetime(2024, 1, 1, 11, 0), "off")

        # Recent pattern: "idle" at 10:00 (after decay period)
        far_future = datetime(2024, 1, 10, 10, 0)
        await tracker.update(far_future, "idle")
        await tracker.update(far_future + timedelta(hours=1), "active")
...
class TestRealisticWorkflows:
    ...
    async def test_home_automation_daily_pattern(self) -> None:
        ...
        for day in range(7):
            ...
            await tracker.update(base, "sleeping")
            # Morning: home (7:00-9:00)
            await tracker.update(base + timedelta(hours=7), "home")
            # Day: away (9:00-17:00)
            await tracker.update(base + timedelta(hours=9), "away")
            # Evening: home (17:00-23:00)
            await tracker.update(base + timedelta(hours=17), "home")
            # Night: sleeping (23:00-24:00)
            await tracker.update(base + timedelta(hours=23), "sleeping")

        # Predictions should match pattern
        pred_morning = await forecaster.predict(datetime(2024, 1, 8, 8, 0))
        assert pred_morning.state == "home"

        pred_midday = await forecaster.predict(datetime(2024, 1, 8, 12, 0))
    ...
    async def test_thermostat_control_pattern(self) -> None:
        ...
        for day in range(5):
            ...
            await tracker.update(base, "idle")
            # Morning warmup: heating (6:00-8:00)
            await tracker.update(base + timedelta(hours=6), "heating")
            # Day: idle (8:00-16:00)
            await tracker.update(base + timedelta(hours=8), "idle")
            # Evening warmup: heating (16:00-18:00)
            await tracker.update(base + timedelta(hours=16), "heating")
            # Night: idle (18:00-24:00)
            await tracker.update(base + timedelta(hours=18), "idle")

        # Should predict heating during warmup periods
        pred = await forecaster.predict(datetime(2024, 1, 6, 7, 0))
        assert pred.state == "heating"

    @pytest.mark.asyncio


    async def test_long_term_tracking(self) -> None:
        ...
        for week in range(4):
            for day in range(7):
                ...
                await tracker.update(date, state)
                await tracker.update(date + timedelta(hours=8), "rest")

        # Pattern should be well-established
        weekday_pred = await forecaster.predict(datetime(2024, 2, 1, 10, 0))
    ...
    async def test_irregular_updates(self) -> None:
        ...
        await tracker.update(datetime(2024, 1, 1, 10, 0), "on")
        await tracker.update(datetime(2024, 1, 1, 10, 5), "off")
        await tracker.update(datetime(2024, 1, 1, 12, 5), "on")
        await tracker.update(datetime(2024, 1, 1, 12, 35), "off")
        await tracker.update(datetime(2024, 1, 1, 15, 35), "on")
...
class TestEdgeCases:
    ...
    async def test_very_first_update_only(self) -> None:
        """Test tracker with only one update ever."""
        indexer = CompositeIndexer([TimeOfDayIndexer(60)])
        forecaster = TimeAwareForecaster(indexer)
        tracker = StateTracker(forecaster)

        await tracker.update(datetime(2024, 1, 1, 10, 0), "single")
    ...
    async def test_extreme_future_timestamp(self) -> None:
        """Test with very far future timestamp."""
        indexer = CompositeIndexer([TimeOfDayIndexer(60)])
        forecaster = TimeAwareForecaster(indexer)
        tracker = StateTracker(forecaster)

        await tracker.update(datetime(2024, 1, 1, 10, 0), "now")
        await tracker.update(datetime(2100, 12, 31, 23, 59), "future")
    ...
    async def test_transition_at_exact_hour(self) -> None:
        """Test state transition at exact hour boundary."""
        indexer = CompositeIndexer([TimeOfDayIndexer(60)])
        forecaster = TimeAwareForecaster(indexer)
        tracker = StateTracker(forecaster)

        await tracker.update(datetime(2024, 1, 1, 10, 0, 0), "on")
        await tracker.update(datetime(2024, 1, 1, 11, 0, 0), "off")
        await tracker.update(datetime(2024, 1, 1, 12, 0, 0), "on")
    ...
    async def test_rapid_state_changes(self) -> None:
        ...
        for i, state in enumerate(states):
            await tracker.update(base + timedelta(seconds=i), state)
