...
class TestInitialization:
    ...
    def test_init_with_simple_indexer(self) -> None:
    ...
    def test_init_with_default_half_life(self) -> None:
...
class TestUpdateInterval:
    ...
    def test_update_single_bucket(self) -> None:
    ...
    def test_update_crossing_bucket_boundary(self) -> None:
        ...
        pred_before = forecaster.predict(datetime(2024, 1, 1, 9, 50))
        pred_after = forecaster.predict(datetime(2024, 1, 1, 10, 10))

        assert pred_before is not None
        assert pred_after is not None

    def test_update_multiple_buckets(self) -> None:
        ...
        pred_9 = forecaster.predict(datetime(2024, 1, 1, 9, 30))
        pred_10 = forecaster.predict(datetime(2024, 1, 1, 10, 30))
        pred_11 = forecaster.predict(datetime(2024, 1, 1, 11, 30))

        assert pred_9.state == "on"
        assert pred_10.state == "on"
        assert pred_11.state == "on"

    def test_update_invalid_interval_end_before_start(self) -> None:
    ...
    def test_update_invalid_interval_end_equals_start(self) -> None:
    ...
    def test_update_multiple_states_same_bucket(self) -> None:
        ...
        forecaster.update_interval(start1, end1, "on")

        start2 = datetime(2024, 1, 2, 10, 0)
        end2 = datetime(2024, 1, 2, 10, 15)
        forecaster.update_interval(start2, end2, "off")

        # Prediction should reflect both observations
        prediction = forecaster.predict(datetime(2024, 1, 3, 10, 15))
        assert prediction is not None
        assert prediction.state in ["on", "off"]
        assert len(prediction.distribution) == 2

    def test_update_crossing_midnight(self) -> None:
        ...
        pred_before = forecaster.predict(datetime(2024, 1, 1, 23, 45))
        pred_after = forecaster.predict(datetime(2024, 1, 2, 0, 15))

        assert pred_before.state == "on"
        assert pred_after.state == "on"

    def test_update_crossing_month_boundary(self) -> None:
        ...
        pred_jan = forecaster.predict(datetime(2024, 1, 31, 23, 45))
        pred_feb = forecaster.predict(datetime(2024, 2, 1, 0, 15))

        assert pred_jan.state == "on"
        assert pred_feb.state == "on"

    def test_update_very_long_interval(self) -> None:
        ...
        for hour in range(24):
            pred = forecaster.predict(datetime(2024, 1, 1, hour, 30))
            assert pred.state == "on"

    def test_update_very_short_interval(self) -> None:
...
class TestPredict:
    ...
    def test_predict_after_multiple_updates(self) -> None:
        ...
        prediction = forecaster.predict(datetime(2024, 1, 3, 10, 30))
    ...
    def test_predict_with_day_of_week_indexer(self) -> None:
        ...
        pred_monday = forecaster.predict(datetime(2024, 1, 8, 10, 30))
        # Predict for next Saturday
        pred_saturday = forecaster.predict(datetime(2024, 1, 13, 10, 30))
    ...
    def test_predict_with_hierarchical_indexer(self) -> None:
        ...
        pred_weekday = forecaster.predict(datetime(2024, 1, 8, 9, 30))
        # Predict for next Saturday morning
        pred_weekend = forecaster.predict(datetime(2024, 1, 13, 9, 30))
...
class TestEdgeCases:
    ...
    def test_exact_bucket_boundary_start(self) -> None:
        ...
        prediction = forecaster.predict(datetime(2024, 1, 1, 10, 15))
        assert prediction.state == "on"

    def test_exact_bucket_boundary_end(self) -> None:
        ...
        prediction = forecaster.predict(datetime(2024, 1, 1, 9, 45))
        assert prediction.state == "on"

    def test_exact_bucket_boundary_both(self) -> None:
    ...
    def test_very_fine_grained_indexer(self) -> None:
        ...
        pred_15 = forecaster.predict(datetime(2024, 1, 1, 10, 15, 30))
        pred_16 = forecaster.predict(datetime(2024, 1, 1, 10, 16, 30))

        assert pred_15.state == "on"
        assert pred_16.state == "on"

    def test_very_coarse_grained_indexer(self) -> None:
        ...
        pred_morning = forecaster.predict(datetime(2024, 1, 1, 9, 0))
        pred_afternoon = forecaster.predict(datetime(2024, 1, 1, 15, 0))
    ...
    def test_leap_year_handling(self) -> None:
        ...
        prediction = forecaster.predict(datetime(2024, 2, 29, 10, 30))
        assert prediction.state == "on"
    ...
    def test_dst_transition(self) -> None:
        ...
        prediction = forecaster.predict(datetime(2024, 3, 10, 2, 30))
        assert prediction is not None
...
class TestIntegration:
    ...
    def test_realistic_daily_pattern(self) -> None:
        ...
        pred_morning = forecaster.predict(datetime(2024, 1, 8, 6, 0))
        pred_work = forecaster.predict(datetime(2024, 1, 8, 12, 0))
    ...
    def test_realistic_weekly_pattern(self) -> None:
        ...
        pred_monday = forecaster.predict(datetime(2024, 1, 8, 12, 0))  # Monday
        pred_saturday = forecaster.predict(datetime(2024, 1, 13, 12, 0))  # Saturday

        assert pred_monday.state == "work"
        assert pred_saturday.state == "leisure"

    def test_pattern_update_over_time(self) -> None:
        ...
        prediction2 = forecaster.predict(datetime(2024, 1, 11, 10, 30))
        # With decay, pattern should shift toward "off"
        assert prediction2 is not None

    def test_multiple_concurrent_patterns(self) -> None:
        ...
        pred_morning = forecaster.predict(datetime(2024, 1, 2, 8, 30))
        pred_lunch = forecaster.predict(datetime(2024, 1, 2, 12, 30))
        pred_dinner = forecaster.predict(datetime(2024, 1, 2, 18, 30))
