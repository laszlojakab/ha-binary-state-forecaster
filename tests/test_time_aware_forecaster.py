...
class MockTimeIndexer:
    ...
    def __init__(self, has_boundary: bool = False):
    ...
    async def next_boundary(self, timestamp: datetime) -> datetime:
        ...
        if not self._has_boundary:
            raise AttributeError("next_boundary not implemented")
...
@pytest.fixture
def mock_indexer_with_boundary():
    """Fixture providing a mock time indexer with boundary support."""
    return MockTimeIndexer(has_boundary=True)
...
@pytest.fixture
def time_aware_params(mock_indexer, engine_params):
...
@pytest.fixture
def forecaster(time_aware_params):
...
def test_parameters_initialization(mock_indexer, engine_params):
...
def test_forecaster_initialization(forecaster, time_aware_params):
...
def test_forecaster_initialization_creates_engine(time_aware_params):
...
@pytest.mark.asyncio
async def test_update_with_datetime(forecaster):
...
@pytest.mark.asyncio
async def test_update_calls_indexer(mock_indexer, forecaster):
...
@pytest.mark.asyncio
async def test_update_multiple_states(forecaster):
...
@pytest.mark.asyncio
async def test_update_different_hours(forecaster):
...
@pytest.mark.asyncio
async def test_predict_with_datetime(forecaster):
...
@pytest.mark.asyncio
async def test_predict_no_prior_data(forecaster):
...
@pytest.mark.asyncio
async def test_predict_returns_result(forecaster):
...
@pytest.mark.asyncio
async def test_predict_different_time_keys(forecaster):
...
@pytest.mark.asyncio
async def test_predict_interval_basic(forecaster):
...
@pytest.mark.asyncio
async def test_predict_interval_multiple_steps(forecaster):
...
@pytest.mark.asyncio
async def test_predict_interval_with_state_simulation(forecaster):
...
@pytest.mark.asyncio
async def test_predict_interval_without_state_simulation(forecaster):
...
@pytest.mark.asyncio
async def test_predict_interval_respects_resolution(forecaster):
...
@pytest.mark.asyncio
async def test_predict_interval_with_boundary_support(time_aware_params, mock_indexer_with_boundary):
    """Test predict_interval with an indexer that supports boundaries."""
    # Create forecaster with boundary-supporting indexer
    params = TimeAwareForecasterParameters(
        indexer=mock_indexer_with_boundary,
...
@pytest.mark.asyncio
async def test_predict_interval_empty_range(forecaster):
...
@pytest.mark.asyncio
async def test_predict_interval_very_small_resolution(forecaster):
...
@pytest.mark.asyncio
async def test_full_workflow(forecaster):
...
@pytest.mark.asyncio
async def test_temporal_context_isolation(forecaster):
...
@pytest.mark.asyncio
async def test_handles_timezone_aware_datetimes(forecaster):
...
@pytest.mark.asyncio
async def test_predict_at_exact_update_time(forecaster):
...
@pytest.mark.asyncio
async def test_rapid_updates_same_time_key(forecaster):
