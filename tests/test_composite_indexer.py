...
class TestCompositeIndexerInitialization:
    ...
    async def test_initialization_with_single_indexer(self) -> None:
    ...
    async def test_initialization_with_multiple_indexers(self) -> None:
    ...
    async def test_initialization_preserves_order(self) -> None:
    ...
    async def test_initialization_with_empty_list(self) -> None:
...
class TestCompositeIndexerKey:
    ...
    async def test_key_single_indexer(self) -> None:
    ...
    async def test_key_two_indexers(self) -> None:
    ...
    async def test_key_structure(self) -> None:
    ...
    async def test_key_preserves_indexer_order(self) -> None:
        ...
        key1 = await indexer1.key(ts)
        key2 = await indexer2.key(ts)
    ...
    async def test_key_different_times_same_day(self) -> None:
    ...
    async def test_key_different_days_same_time(self) -> None:
    ...
    async def test_key_completely_different_timestamps(self) -> None:
    ...
    async def test_key_same_composite_bucket(self) -> None:
    ...
    async def test_key_empty_indexers(self) -> None:
...
class TestCompositeIndexerNextBoundary:
    ...
    async def test_next_boundary_single_indexer(self) -> None:
    ...
    async def test_next_boundary_takes_minimum(self) -> None:
    ...
    async def test_next_boundary_day_comes_first(self) -> None:
    ...
    async def test_next_boundary_multiple_indexers_same_time(self) -> None:
    ...
    async def test_next_boundary_three_indexers(self) -> None:
    ...
    async def test_next_boundary_empty_indexers(self) -> None:
...
class TestCompositeIndexerEdgeCases:
    ...
    async def test_repeated_indexer_types(self) -> None:
    ...
    async def test_key_boundary_consistency(self) -> None:
    ...
    async def test_complex_three_way_composite(self) -> None:
    ...
    async def test_boundary_at_day_and_time_transition(self) -> None:
        """Test boundary when both day and time change simultaneously."""
        indexer = CompositeIndexer([DayOfWeekIndexer(), TimeOfDayIndexer(bucket_minutes=60)])

        # 23:30 on Monday
        ts = datetime(2026, 1, 26, 23, 30)
        key_before = await indexer.key(ts)

        # Next boundary is midnight (both change)
        next_ts = await indexer.next_boundary(ts)
        assert next_ts == datetime(2026, 1, 27, 0, 0)

        key_after = await indexer.key(next_ts)
