...
class TestTimeOfDayIndexerInitialization:
    ...
    def test_initialization_different_buckets(self) -> None:
...
class TestTimeOfDayIndexerKey:
    ...
    def test_key_midnight(self) -> None:
        """Test key at midnight."""
        indexer = TimeOfDayIndexer(bucket_minutes=30)
        ts = datetime(2026, 1, 29, 0, 0, 0)
        assert indexer.key(ts) == 0

    def test_key_first_bucket(self) -> None:
        """Test key in first bucket."""
        indexer = TimeOfDayIndexer(bucket_minutes=30)
        ts = datetime(2026, 1, 29, 0, 15, 0)
        assert indexer.key(ts) == 0

    def test_key_second_bucket(self) -> None:
        """Test key in second bucket."""
        indexer = TimeOfDayIndexer(bucket_minutes=30)
        ts = datetime(2026, 1, 29, 0, 30, 0)
        assert indexer.key(ts) == 1

    def test_key_various_times(self) -> None:
        """Test key calculation for various times."""
        indexer = TimeOfDayIndexer(bucket_minutes=30)

        # 6:00 AM -> bucket 12 (6 * 60 / 30 = 12)
        assert indexer.key(datetime(2026, 1, 29, 6, 0, 0)) == 12

        # 6:29 AM -> still bucket 12
        assert indexer.key(datetime(2026, 1, 29, 6, 29, 0)) == 12

        # 6:30 AM -> bucket 13
        assert indexer.key(datetime(2026, 1, 29, 6, 30, 0)) == 13

        # 12:00 PM (noon) -> bucket 24 (12 * 60 / 30 = 24)
        assert indexer.key(datetime(2026, 1, 29, 12, 0, 0)) == 24

        # 23:59 PM -> bucket 47 (23 * 60 + 59 = 1439, 1439 // 30 = 47)
        assert indexer.key(datetime(2026, 1, 29, 23, 59, 0)) == 47

    def test_key_15_minute_buckets(self) -> None:
        """Test key calculation with 15-minute buckets."""
        indexer = TimeOfDayIndexer(bucket_minutes=15)

        # 6:00 AM -> bucket 24 (6 * 60 / 15 = 24)
        assert indexer.key(datetime(2026, 1, 29, 6, 0, 0)) == 24

        # 6:14 AM -> still bucket 24
        assert indexer.key(datetime(2026, 1, 29, 6, 14, 0)) == 24

        # 6:15 AM -> bucket 25
        assert indexer.key(datetime(2026, 1, 29, 6, 15, 0)) == 25

        # 6:30 AM -> bucket 26
        assert indexer.key(datetime(2026, 1, 29, 6, 30, 0)) == 26

        # 6:45 AM -> bucket 27
        assert indexer.key(datetime(2026, 1, 29, 6, 45, 0)) == 27

    def test_key_60_minute_buckets(self) -> None:
        """Test key calculation with 60-minute (hourly) buckets."""
        indexer = TimeOfDayIndexer(bucket_minutes=60)

        # 6:00 AM -> bucket 6
        assert indexer.key(datetime(2026, 1, 29, 6, 0, 0)) == 6

        # 6:59 AM -> still bucket 6
        assert indexer.key(datetime(2026, 1, 29, 6, 59, 0)) == 6

        # 7:00 AM -> bucket 7
        assert indexer.key(datetime(2026, 1, 29, 7, 0, 0)) == 7

        # 23:00 PM -> bucket 23
        assert indexer.key(datetime(2026, 1, 29, 23, 0, 0)) == 23

    def test_key_ignores_date(self) -> None:
        ...
        assert indexer.key(ts1) == indexer.key(ts2) == indexer.key(ts3)

    def test_key_ignores_seconds(self) -> None:
...
class TestTimeOfDayIndexerNextBoundary:
    ...
    def test_next_boundary_at_bucket_start(self) -> None:
    ...
    def test_next_boundary_middle_of_bucket(self) -> None:
    ...
    def test_next_boundary_near_end_of_bucket(self) -> None:
    ...
    def test_next_boundary_15_minute_buckets(self) -> None:
        """Test next_boundary with 15-minute buckets."""
        indexer = TimeOfDayIndexer(bucket_minutes=15)

        # From 6:00 -> 6:15
        assert indexer.next_boundary(datetime(2026, 1, 29, 6, 0, 0)) == datetime(
            2026, 1, 29, 6, 15, 0
        )

        # From 6:07 -> 6:15
        assert indexer.next_boundary(datetime(2026, 1, 29, 6, 7, 0)) == datetime(
            2026, 1, 29, 6, 15, 0
        )

        # From 6:15 -> 6:30
        assert indexer.next_boundary(datetime(2026, 1, 29, 6, 15, 0)) == datetime(
            2026, 1, 29, 6, 30, 0
        )

        # From 6:30 -> 6:45
        assert indexer.next_boundary(datetime(2026, 1, 29, 6, 30, 0)) == datetime(
            2026, 1, 29, 6, 45, 0
        )

    def test_next_boundary_60_minute_buckets(self) -> None:
        ...
        # From 6:59 -> 7:00
        assert indexer.next_boundary(datetime(2026, 1, 29, 6, 59, 59)) == datetime(
            2026, 1, 29, 7, 0, 0
        )

    def test_next_boundary_midnight_wrap(self) -> None:
    ...
    def test_next_boundary_removes_seconds(self) -> None:
    ...
    def test_next_boundary_preserves_date(self) -> None:
...
class TestTimeOfDayIndexerEdgeCases:
    ...
    def test_large_bucket_size(self) -> None:
        ...
        # 0:00 -> bucket 0
        assert indexer.key(datetime(2026, 1, 29, 0, 0, 0)) == 0

        # 1:59 -> still bucket 0
        assert indexer.key(datetime(2026, 1, 29, 1, 59, 0)) == 0

        # 2:00 -> bucket 1
        assert indexer.key(datetime(2026, 1, 29, 2, 0, 0)) == 1

        # 6:00 -> bucket 3 (6 * 60 / 120 = 3)
        assert indexer.key(datetime(2026, 1, 29, 6, 0, 0)) == 3
    ...
    def test_key_consistency_across_boundaries(self) -> None:
        ...
        next_bound = indexer.next_boundary(ts1)
        assert indexer.key(next_bound) == key2
