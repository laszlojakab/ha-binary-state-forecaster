...
class TestStatePersistenceTracker:
    ...
    async def test_update_single_persistence(self) -> None:
    ...
    async def test_update_single_transition(self) -> None:
    ...
    async def test_update_multiple_observations(self) -> None:
    ...
    async def test_get_persistence_factor_insufficient_data(self) -> None:
    ...
    async def test_get_persistence_factor_sufficient_data(self) -> None:
    ...
    async def test_get_persistence_factor_low_persistence(self) -> None:
    ...
    async def test_get_persistence_factor_mixed(self) -> None:
    ...
    async def test_multiple_states(self) -> None:
...
class TestAdaptivePersistence:
    ...
    async def test_adaptive_persistence_enabled_by_default(self) -> None:
    ...
    async def test_adaptive_persistence_can_be_disabled(self) -> None:
    ...
    async def test_learns_state_persistence(self) -> None:
    ...
    async def test_get_learned_persistence_all_states(self) -> None:
    ...
    async def test_get_learned_persistence_specific_state(self) -> None:
...
class TestPredictionWithCurrentState:
    ...
    async def test_predict_without_current_state(self) -> None:
    ...
    async def test_predict_with_matching_current_state(self) -> None:
        ...
        # Predict without current state
        pred_no_context = await forecaster.predict(datetime(2024, 1, 3, 10, 30))

        # Predict with current state = "on"
        pred_with_on = await forecaster.predict(
            datetime(2024, 1, 3, 10, 30), current_state="on"
        )

        # Predict with current state = "off"
        pred_with_off = await forecaster.predict(
    ...
    async def test_predict_with_novel_current_state(self) -> None:
...
class TestDurationDependentPersistence:
    ...
    async def test_duration_adjustment_short_duration(self) -> None:
    ...
    async def test_duration_adjustment_long_duration(self) -> None:
        ...
        pred_long = await forecaster.predict(
    ...
    async def test_duration_adjustment_scaling(self) -> None:
        ...
        pred_5min = await forecaster.predict(
            datetime(2024, 1, 3, 10, 30),
            current_state="on",
            state_duration=300.0,  # 5 minutes
        )
        pred_30min = await forecaster.predict(
            datetime(2024, 1, 3, 10, 30),
            current_state="on",
            state_duration=1800.0,  # 30 minutes
        )
        pred_2hr = await forecaster.predict(
            datetime(2024, 1, 3, 10, 30),
            current_state="on",
            state_duration=7200.0,  # 2 hours
        )
        pred_10hr = await forecaster.predict(
            datetime(2024, 1, 3, 10, 30),
            current_state="on",
    ...
    async def test_duration_adjustment_caps_at_one(self) -> None:
    ...
    async def test_duration_none_uses_base_factor(self) -> None:
    ...
    async def test_duration_zero_uses_base_factor(self) -> None:
        ...
        pred_zero = await forecaster.predict(
            datetime(2024, 1, 2, 10, 30), current_state="on", state_duration=0.0
        )

        # Predict without duration
        pred_none = await forecaster.predict(datetime(2024, 1, 2, 10, 30), current_state="on")
...
class TestIntegrationAdaptiveAndDuration:
    ...
    async def test_adaptive_and_duration_together(self) -> None:
    ...
    async def test_fixed_persistence_with_duration(self) -> None:
