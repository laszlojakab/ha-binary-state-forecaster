...
class TestDayOfWeekIndexerEdgeCases:
    ...
    async def test_leap_year_february_29(self) -> None:
        ...
        assert await indexer.key(ts) == 3  # Thursday
        next_bound = await indexer.next_boundary(ts)
    ...
    async def test_same_time_different_years(self) -> None:
        ...
        assert await indexer.key(ts_2026) == 0  # Monday
        assert await indexer.key(ts_2027) == 1  # Tuesday
        assert await indexer.key(ts_2028) == 2  # Wednesday
